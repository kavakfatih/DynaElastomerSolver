program test_q4_fbar_force_solver
  ...
  real(dp), parameter :: tol_reaction = 2.0e-7_dp
  ...
  if (status /= DES_STATUS_OK) error stop 'F-bar analitik traction assembly başarısız.'
  ...
  if (report%increments_converged /= 5) error stop 'F-bar tüm incrementleri tamamlamadı.'
  if (report%min_j <= 0.0_dp) error stop 'F-bar çözümünde J pozitif değil.'

  if (abs(u(2,1)-(lambda_x-1.0_dp)) > tol_u) error stop 'F-bar node2 x displacement hatalı.'
  if (abs(u(3,1)-(lambda_x-1.0_dp)) > tol_u) error stop 'F-bar node3 x displacement hatalı.'
  if (abs(u(3,2)-(lambda_y_ref-1.0_dp)) > tol_u) error stop 'F-bar lateral contraction hatalı.'
  if (abs(u(4,2)-(lambda_y_ref-1.0_dp)) > tol_u) error stop 'F-bar üst sol contraction hatalı.'
...
end program test_q4_fbar_force_solver
