program test_q9_herrmann_mesh_refinement
  ...
  integer, parameter :: mesh_n(nlevels) = [1,2,4]
  real(dp), parameter :: mu = 1.0_dp
  real(dp), parameter :: pressure_compliance = 1.0e-3_dp
  real(dp), parameter :: residual_tol = 5.0e-8_dp
  real(dp), parameter :: final_self_gap_limit = 5.0e-2_dp
  real(dp) :: tip_y(nlevels),disp_norm(nlevels),pressure_norm(nlevels),constraint_norm(nlevels)
  real(dp) :: gap12,gap24
  integer :: i,status,json_unit,ios

  do i = 1,nlevels
    call solve_level( &
        mesh_n(i),tip_y(i),disp_norm(i),pressure_norm(i),constraint_norm(i),status)
    if (status /= DES_STATUS_OK) then
      error stop 'Q9/P1 Cook mesh-refinement seviyesi yakinsamadi.'
    end if
    if (tip_y(i) <= 0.0_dp) then
      error stop 'Q9/P1 Cook midpoint displacement yuk yonuyle uyumsuz.'
    end if
    if (disp_norm(i) > residual_tol .or. pressure_norm(i) > residual_tol) then
      error stop 'Q9/P1 Cook mesh-refinement weak residual tolerans disi.'
    end if
    if (constraint_norm(i) < 0.0_dp .or. constraint_norm(i) >= huge(1.0_dp)) then
      error stop 'Q9/P1 Cook mesh-refinement volumetric diagnostic gecersiz.'
    end if
  end do

  gap12 = relative_gap(tip_y(1),tip_y(2))
  gap24 = relative_gap(tip_y(2),tip_y(3))

  ! Bu ilk self-convergence gate external mixed referansin yerine gecmez. Amac;
  ! Q9/P1 solution sequence'in mesh incelmesiyle duzenli davranmasini ve 2x2 -> 4x4
  ! degisiminin kontrol altinda kalmasini regression olarak korumaktir.
  if (gap24 > gap12) then
    error stop 'Q9/P1 Cook mesh refinement degisimi incelmeyle azalmiyor.'
  end if
  if (gap24 > final_self_gap_limit) then
    error stop 'Q9/P1 2x2 -> 4x4 midpoint displacement self-gap %5 sinirini asti.'
  end if

  write(*,'(A)') 'Q9/P1 Herrmann Cook midpoint mesh-refinement (K/mu=1000, 3x3):'
  do i = 1,nlevels
    write(*,'(A,I0,A,ES14.6,A,ES12.4,A,ES12.4,A,ES12.4)') &
        '  mesh=',mesh_n(i),'x',tip_y(i), &
        ' Ru_inf=',disp_norm(i),' Rp_inf=',pressure_norm(i),' Cvol_inf=',constraint_norm(i)
  end do
  write(*,'(A,F9.4,A)') '  1x1 -> 2x2 midpoint gap = ',100.0_dp*gap12,' %'
  write(*,'(A,F9.4,A)') '  2x2 -> 4x4 midpoint gap = ',100.0_dp*gap24,' %'

  open(newunit=json_unit,file='V0.3_HERRMANN_MESH_REFINEMENT_RESULTS.json', &
       status='replace',action='write',iostat=ios)
  if (ios /= 0) error stop 'Q9/P1 mesh-refinement JSON acilamadi.'
  write(json_unit,'(A)') '{'
  write(json_unit,'(A)') '  "schema_version": 1,'
  write(json_unit,'(A)') '  "benchmark": "q9_p1_herrmann_cook_mesh_refinement",'
  write(json_unit,'(A)') '  "tip_definition": "right_edge_geometric_midpoint",'
  write(json_unit,'(A)') '  "quadrature": "3x3",'
  write(json_unit,'(A)') '  "bulk_over_mu": 1000,'
  write(json_unit,'(A,I0,A,I0,A,I0,A)') &
      '  "mesh_n": [',mesh_n(1),', ',mesh_n(2),', ',mesh_n(3),'],'
  write(json_unit,'(A,ES24.16,A,ES24.16,A,ES24.16,A)') &
      '  "tip_y": [',tip_y(1),', ',tip_y(2),', ',tip_y(3),'],'
  write(json_unit,'(A,ES24.16,A)') '  "gap_1_to_2": ',gap12,','
  write(json_unit,'(A,ES24.16)') '  "gap_2_to_4": ',gap24
  write(json_unit,'(A)') '}'
  close(json_unit)

  write(*,'(A)') 'Q9/P1 Herrmann mesh-refinement self-convergence gate BASARILI.'

contains

  subroutine solve_level(n,tip,du_metric,p_metric,c_metric,solve_status)
    integer, intent(in) :: n
    real(dp), intent(out) :: tip,du_metric,p_metric,c_metric
    ...
    ! Cook benchmark sonucu sag kenarin geometrik orta noktasinda okunur. Bu tanim
    ! FEniCSx external reference ile aynidir; top-right corner "tip" olarak kullanilmaz.
    npx = 2*n+1
...
end program test_q9_herrmann_mesh_refinement
