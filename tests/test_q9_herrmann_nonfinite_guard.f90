program test_q9_herrmann_nonfinite_guard
  ...
  if (report%nonlinear%status /= DES_ERROR_NONFINITE_NONLINEAR) then
    error stop 'NaN external load beklenen nonlinear non-finite statusunu vermedi.'
  end if
  if (report%nonlinear%last_failure_status /= DES_ERROR_NONFINITE_NONLINEAR) then
    error stop 'NaN external load last_failure_status alanina tasinmadi.'
  end if
  if (report%nonlinear%increments_attempted /= 0) then
    error stop 'NaN external load ile Newton increment denemesi baslatildi.'
  end if
  if (report%nonlinear%cutback_count /= 0) then
    error stop 'NaN input physics failure gibi cutback zincirine sokuldu.'
  end if
  if (report%nonlinear%state_commit_count /= 0 .or. &
      report%nonlinear%state_revert_count /= 0) then
    error stop 'NaN input state transaction sayaclarini degistirdi.'
  end if
  if (report%nonlinear%history%count /= 0) then
    error stop 'Fail-fast NaN input convergence history olusturdu.'
  end if
  if (report%nonfinite_event_count /= 1) then
    error stop 'Production Herrmann report NaN/Inf olayini ozetlemedi.'
  end if
