program test_q9_herrmann_distorted_pressure_stability
  ...
  call set_distorted_2x2_q9(X,conn)
    ...
    do gy = 1,3
      do gx = 1,3
        ...
        if (status /= DES_STATUS_OK) then
          error stop 'Distorted Q9/P1 pressure stability mesh Jacobiani gecersiz.'
        end if
  ...
  if (minimum_det_jac <= 0.0_dp) then
    error stop 'Distorted Q9 mesh pozitif Jacobian kosulunu saglamiyor.'
  end if
  if (jacobian_spread < 6.0_dp) then
    error stop 'Distorted Q9 pressure testi yeterli geometrik distortion icermiyor.'
  end if
  ...
  if (rank_value /= npdof) then
    error stop 'Distorted Q9/P1 pressure coupling tam row rank degil.'
  end if
  ...
  if (normalized_checker_coupling < 0.25_dp) then
    error stop 'Distorted Q9/P1 alternating pressure mode zayif baglaniyor.'
  end if

  write(*,'(A,ES14.6)') 'Distorted Q9/P1 minimum det(J) = ',minimum_det_jac
  write(*,'(A,ES14.6)') 'Distorted Q9/P1 maximum det(J) = ',maximum_det_jac
  write(*,'(A,ES14.6)') 'Distorted Q9/P1 Jacobian spread = ',jacobian_spread
  write(*,'(A,I0,A,I0)') 'Distorted Q9/P1 pressure coupling rank = ', &
      rank_value,' / ',npdof
  write(*,'(A,ES14.6)') 'Distorted Q9/P1 normalized checkerboard coupling = ', &
      normalized_checker_coupling
  write(*,'(A)') 'Distorted Q9/P1 pressure stability testi BASARILI.'
...
contains
  ...
  subroutine set_distorted_2x2_q9(coords,connectivity)
    ...
    ! Smooth nonlinear warp: shared Q9 dugumleri tek bir global mapping ile tasinir.
    ! Sol sinir x=0 olarak korunur; ic/sag alan bilincli bicimde agir bozulur.
    node = 0
    do iy = 0,4
      do ix = 0,4
        node = node+1
...
end program test_q9_herrmann_distorted_pressure_stability
