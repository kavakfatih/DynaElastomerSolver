program test_q9_herrmann_infsup_proxy
  ...
  real(dp) :: beta(ncases), plateau_ratio, tail_change
  integer :: i

  do i = 1,ncases
    call evaluate_infsup_proxy(mesh_size(i),beta(i))
    write(*,'(A,I0,A,ES14.6)') 'Q9/P1 inf-sup proxy n=',mesh_size(i),' beta_h=',beta(i)
  end do

  plateau_ratio = beta(ncases)/beta(1)
  tail_change = abs(beta(ncases)-beta(ncases-1))/beta(ncases-1)

  if (minval(beta) < 0.55_dp) then
    error stop 'Q9/P1 discrete inf-sup proxy kabul alt sinirinin altina dustu.'
  end if
  if (plateau_ratio < 0.80_dp) then
    error stop 'Q9/P1 inf-sup proxy mesh incelmesiyle kabul edilemez bicimde cokuyor.'
  end if
  if (tail_change > 0.12_dp) then
    error stop 'Q9/P1 inf-sup proxy henuz mesh-independent plateau davranisi gostermiyor.'
  end if

  write(*,'(A,ES14.6)') 'Q9/P1 beta(n=4)/beta(n=2) = ',plateau_ratio
  write(*,'(A,ES14.6)') 'Q9/P1 tail relative change = ',tail_change
  write(*,'(A)') 'Q9/P1 discrete inf-sup proxy mesh-trend testi BASARILI.'
...
contains
  ...
  subroutine evaluate_infsup_proxy(mesh_n,beta_h)
    ...
    real(dp) :: Nshape(9),dN_parent(9,2),dN_dX(9,2),Np(3)
    ...
    allocate(X(nnode,2),conn(nelem,9))
    ...
    call build_structured_q9_mesh(mesh_n,X,conn)
    Kdisp = 0.0_dp
    Bcouple = 0.0_dp
    Mp = 0.0_dp
      ...
      do gy = 1,3
        do gx = 1,3
          ...
          if (status /= DES_STATUS_OK) then
            error stop 'Q9/P1 inf-sup mesh Jacobiani gecersiz.'
          end if
    ...
    ! Sol sinirda iki displacement component sabitlenir.
    do iy = 0,2*mesh_n
      node_a = iy*ngrid+1
    ...
    do q = 1,npdof
      ...
      if (.not. ok) then
        error stop 'Q9/P1 inf-sup proxy displacement norm sistemi cozulmedi.'
      end if
      response(:,q) = solution
    end do
    ...
    if (.not. ok) error stop 'Q9/P1 pressure mass matrisi pozitif tanimli degil.'
    ...
    if (.not. ok) error stop 'Q9/P1 inf-sup eigenvalue iterasyonu yakinsamadi.'
    if (minval(eigenvalues) <= 1.0e-12_dp) then
      error stop 'Q9/P1 pressure space pozitif discrete inf-sup eigenvalue uretmedi.'
    end if

    beta_h = sqrt(minval(eigenvalues))
  end subroutine evaluate_infsup_proxy

  subroutine build_structured_q9_mesh(n,coords,connectivity)
    integer, intent(in) :: n
    real(dp), intent(out) :: coords(:,:)
    integer, intent(out) :: connectivity(:,:)
    integer :: ngrid,ix,iy,ex,ey,e,i0,j0,node
    real(dp) :: scale
    ...
    do ey = 0,n-1
      do ex = 0,n-1
        ...
        i0 = 2*ex
        j0 = 2*ey
        connectivity(e,1) = node_id(i0,  j0,  ngrid)
        connectivity(e,2) = node_id(i0+2,j0,  ngrid)
        connectivity(e,3) = node_id(i0+2,j0+2,ngrid)
        connectivity(e,4) = node_id(i0,  j0+2,ngrid)
        connectivity(e,5) = node_id(i0+1,j0,  ngrid)
        connectivity(e,6) = node_id(i0+2,j0+1,ngrid)
        connectivity(e,7) = node_id(i0+1,j0+2,ngrid)
        connectivity(e,8) = node_id(i0,  j0+1,ngrid)
        connectivity(e,9) = node_id(i0+1,j0+1,ngrid)
      end do
    end do
  end subroutine build_structured_q9_mesh

  pure integer function node_id(ix,iy,ngrid) result(id)
    integer, intent(in) :: ix,iy,ngrid
    id = iy*ngrid+ix+1
...
end program test_q9_herrmann_infsup_proxy
