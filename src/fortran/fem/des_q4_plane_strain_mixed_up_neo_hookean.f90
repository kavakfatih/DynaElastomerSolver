module des_q4_plane_strain_mixed_up_neo_hookean
  use des_kinds, only : dp
  use des_status, only : DES_STATUS_OK, DES_ERROR_INVALID_PARAMETERS, &
                         DES_ERROR_SINGULAR_F, DES_ERROR_NONPOSITIVE_J, &
                         DES_ERROR_INVALID_ELEMENT_JACOBIAN
  use des_tensor3, only : inverse3
  use des_q4_shape, only : q4_shape_functions
  implicit none
  private

  public :: evaluate_q4_plane_strain_mixed_up_element

contains

  pure subroutine evaluate_q4_plane_strain_mixed_up_element( &
      X, u, pressure, mu, lame_lambda, residual, tangent, status, min_j)
    ! İlk V0.3 mixed u-p prototipi:
    ! - Q4 displacement (8 DOF)
    ! - eleman başına sabit P0 pressure (1 DOF)
    !
    ! Mixed potansiyel yoğunluğu:
    ! Psi(F,p) = mu/2*(I1-3) - mu*ln(J) + p*ln(J) - p^2/(2*lambda)
    !
    ! Pressure stationarity:
    ! dPsi/dp = ln(J) - p/lambda = 0
    ! Dolayısıyla noktasal olarak p=lambda*ln(J) seçilirse mevcut compressible
    ! Neo-Hookean enerji ve gerilme birebir geri elde edilir. P0 interpolation
    ! pressure'ı eleman ölçeğinde ortalama volumetrik değişken haline getirir.
    real(dp), intent(in) :: X(4,2), u(4,2)
    real(dp), intent(in) :: pressure, mu, lame_lambda
    real(dp), intent(out) :: residual(9), tangent(9,9)
    integer, intent(out) :: status
    real(dp), intent(out) :: min_j

    real(dp), parameter :: gp = 0.57735026918962576451_dp
    real(dp), parameter :: gauss_xi(4) = [-gp, gp, gp, -gp]
    real(dp), parameter :: gauss_eta(4) = [-gp, -gp, gp, gp]
    real(dp), parameter :: jac_tol = 100.0_dp*epsilon(1.0_dp)

    real(dp) :: N(4), dN_parent(4,2), dN_dX(4,2)
    real(dp) :: Jmap(2,2), invJmap(2,2), detJmap
    real(dp) :: F(3,3), Finv(3,3), FinvT(3,3)
    real(dp) :: P(3,3), A(3,3,3,3)
    real(dp) :: J, lnJ, weight
    integer :: g, a, b, i, k, Jdir, Ldir, row, col
    logical :: inverse_ok

    residual = 0.0_dp
    tangent = 0.0_dp
    status = DES_STATUS_OK
    min_j = huge(1.0_dp)

    if (mu <= 0.0_dp .or. lame_lambda <= 0.0_dp) then
      status = DES_ERROR_INVALID_PARAMETERS
      return
    end if

    do g = 1,4
      call q4_shape_functions(gauss_xi(g), gauss_eta(g), N, dN_parent)
      call reference_gradient(X, dN_parent, Jmap, invJmap, detJmap, dN_dX)
      if (detJmap <= jac_tol) then
        status = DES_ERROR_INVALID_ELEMENT_JACOBIAN
        return
      end if

      F = 0.0_dp
      F(1,1) = 1.0_dp
      F(2,2) = 1.0_dp
      F(3,3) = 1.0_dp
      do a = 1,4
        do i = 1,2
          do Jdir = 1,2
            F(i,Jdir) = F(i,Jdir) + u(a,i)*dN_dX(a,Jdir)
          end do
        end do
      end do

      call inverse3(F, Finv, J, inverse_ok)
      min_j = min(min_j, J)
      if (.not. inverse_ok) then
        status = DES_ERROR_SINGULAR_F
        return
      end if
      if (J <= 0.0_dp) then
        status = DES_ERROR_NONPOSITIVE_J
        return
      end if

      FinvT = transpose(Finv)
      lnJ = log(J)
      weight = detJmap

      ! P = dPsi/dF = mu*F + (p-mu)*F^{-T}
      P = mu*F + (pressure-mu)*FinvT

      ! A = dP/dF |p sabit.
      A = 0.0_dp
      do i = 1,3
        do Jdir = 1,3
          do k = 1,3
            do Ldir = 1,3
              if (i == k .and. Jdir == Ldir) then
                A(i,Jdir,k,Ldir) = A(i,Jdir,k,Ldir) + mu
              end if
              A(i,Jdir,k,Ldir) = A(i,Jdir,k,Ldir) &
                - (pressure-mu)*FinvT(k,Jdir)*FinvT(i,Ldir)
            end do
          end do
        end do
      end do

      do a = 1,4
        do i = 1,2
          row = 2*(a-1) + i
          do Jdir = 1,2
            residual(row) = residual(row) &
              + P(i,Jdir)*dN_dX(a,Jdir)*weight

            ! K_up = dR_u/dp.
            tangent(row,9) = tangent(row,9) &
              + FinvT(i,Jdir)*dN_dX(a,Jdir)*weight
          end do
        end do
      end do

      ! R_p = integral[ln(J)-p/lambda] dV0.
      residual(9) = residual(9) &
        + (lnJ-pressure/lame_lambda)*weight

      do a = 1,4
        do i = 1,2
          row = 2*(a-1) + i
          do b = 1,4
            do k = 1,2
              col = 2*(b-1) + k
              do Jdir = 1,2
                do Ldir = 1,2
                  tangent(row,col) = tangent(row,col) &
                    + A(i,Jdir,k,Ldir) &
                    * dN_dX(a,Jdir)*dN_dX(b,Ldir)*weight
                end do
              end do
            end do
          end do
        end do
      end do

      ! K_pu = dR_p/du ve K_pp = dR_p/dp.
      do b = 1,4
        do k = 1,2
          col = 2*(b-1) + k
          do Ldir = 1,2
            tangent(9,col) = tangent(9,col) &
              + FinvT(k,Ldir)*dN_dX(b,Ldir)*weight
          end do
        end do
      end do
      tangent(9,9) = tangent(9,9) - weight/lame_lambda
    end do
  end subroutine evaluate_q4_plane_strain_mixed_up_element

  pure subroutine reference_gradient( &
      X, dN_parent, Jmap, invJmap, detJmap, dN_dX)
    real(dp), intent(in) :: X(4,2), dN_parent(4,2)
    real(dp), intent(out) :: Jmap(2,2), invJmap(2,2), detJmap
    real(dp), intent(out) :: dN_dX(4,2)
    integer :: a

    Jmap = 0.0_dp
    do a = 1,4
      Jmap(1,1) = Jmap(1,1) + dN_parent(a,1)*X(a,1)
      Jmap(1,2) = Jmap(1,2) + dN_parent(a,1)*X(a,2)
      Jmap(2,1) = Jmap(2,1) + dN_parent(a,2)*X(a,1)
      Jmap(2,2) = Jmap(2,2) + dN_parent(a,2)*X(a,2)
    end do

    detJmap = Jmap(1,1)*Jmap(2,2) - Jmap(1,2)*Jmap(2,1)
    if (abs(detJmap) <= 100.0_dp*epsilon(1.0_dp)) then
      invJmap = 0.0_dp
      dN_dX = 0.0_dp
      return
    end if

    invJmap(1,1) =  Jmap(2,2)/detJmap
    invJmap(1,2) = -Jmap(1,2)/detJmap
    invJmap(2,1) = -Jmap(2,1)/detJmap
    invJmap(2,2) =  Jmap(1,1)/detJmap

    do a = 1,4
      dN_dX(a,1) = invJmap(1,1)*dN_parent(a,1) &
                  + invJmap(1,2)*dN_parent(a,2)
      dN_dX(a,2) = invJmap(2,1)*dN_parent(a,1) &
                  + invJmap(2,2)*dN_parent(a,2)
    end do
  end subroutine reference_gradient

end module des_q4_plane_strain_mixed_up_neo_hookean
